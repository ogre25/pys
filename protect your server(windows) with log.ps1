...
{
    param($comm)
    $cdate=$(Get-Date).ToString("yyyy-MM-dd-HH-mm-ss")
        if($? -eq $True)
        {
         "$cdate $comm success!!!!"  | out-file -append execute-result.log
         }
        else
        {
         "$cdate $comm fail!!!!!!" | out-file -append execute-result.log
        }
}
  ...
  do
  {
     ...
     switch ($input)
     {
            '1'{
                cls
                chrdpdefport
                writlog -comm "default port is"

            } '2' {
                   cls
                   remevepri
                   writlog -comm "privilege remove"

            } '3' {
                    cls
                    rnameadmin
                    writelog -comm "rename is"

            } '4' {
                    cls
                    distylauser
                    writlog -comm "disable last logon username is"
